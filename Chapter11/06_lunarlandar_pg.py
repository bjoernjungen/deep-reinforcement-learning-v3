...
if __name__ == "__main__":
    ...
    env = RecordVideo(env=gym.make(game, render_mode="rgb_array", continuous=False, gravity=-10.0,
             enable_wind=True, wind_power=15.0, turbulence_power=1.5), video_folder="./" + game + "-training-pg",
                      episode_trigger=lambda x: x % 20 == 0 and x > 0)
    env = RecordVideo(env=gym.make(game, render_mode="rgb_array"), video_folder="./" + game + "-training-pg", episode_trigger=lambda x: x % 20 == 0 and x > 0)

    writer = SummaryWriter(comment="-lunarlandar-pg")
